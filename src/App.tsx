...
const MainSearchBar = ({ searchEngine = 'google', onCycleEngine }) => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto mb-10 relative z-10">
      <div className="flex items-center w-full border border-zinc-700/60 bg-zinc-900/60 backdrop-blur-xl px-6 h-14 rounded-2xl shadow-xl focus-within:border-zinc-500 focus-within:bg-zinc-900/80 focus-within:ring-4 focus-within:ring-zinc-500/20 transition-all duration-300">
        ...
        <button 
          ...
          className="hidden md:flex items-center justify-center px-4 py-1.5 bg-zinc-800/50 hover:bg-zinc-700/80 rounded-xl border border-zinc-700/50 transition-all group cursor-pointer ml-2 text-xs font-bold text-zinc-400 hover:text-white capitalize tracking-wider"
        >
          {searchEngine}
        </button>
      </div>
...
const TasksWidget = () => {
  const [tasks, setTasks] = useState(() => {
    ...
    return saved ? JSON.parse(saved) : [
      { id: 1, text: "Review PR #402", done: false },
      { id: 2, text: "Update dark mode styles", done: true },
      { id: 3, text: "Deploy staging env", done: false },
    ];
  });
          ...
          tasks.map(task => (
            ...
            >
              <div className="flex items-center gap-3 overflow-hidden">
                <div className={`w-4 h-4 shrink-0 flex items-center justify-center rounded border transition-all duration-300 ${
                  task.done 
    ...
    <div className="flex flex-col h-[320px] bg-zinc-900/60 border border-zinc-800/80 rounded-3xl p-5 backdrop-blur-xl shadow-lg relative">
      ...
      <div className="relative w-full flex-1 rounded-2xl overflow-hidden group mb-4 shadow-inner bg-zinc-950">
        ...
        <img 
          ...
          alt="Space" 
          className="w-full h-full object-cover transition-all duration-1000 ease-in-out brightness-75 group-hover:brightness-100 group-hover:scale-105"
        />
        ...
        <div className="absolute bottom-3 left-3 right-3">
          ...
          <p className="text-sm text-zinc-400 line-clamp-2 leading-tight">
            A cavernous region of star birth located in the nearby spiral galaxy M33.
          </p>
        </div>
      </div>
  ...
  return (
    <div className="h-screen w-full relative flex flex-col justify-center items-center px-6 md:px-12 selection:bg-zinc-200 selection:text-zinc-900">
      ...
      <div className="w-full max-w-5xl mx-auto flex flex-col items-center relative z-10 pointer-events-auto">
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-1 gap-6 fade-in" style={{ animationDelay: '0.1s' }}>
         <ApodWidget />
          {/* <LinksWidget />
          <TasksWidget /> */}
        </div>
